...
int	main(void)
{
	...
	if (!strcmp(elem->content, str1) \
		...
		&& !strcmp(elem->next->next->next->next->content, "Hello") \
		&& !strcmp(elem->next->next->next->next->next->content, "Hello") \
		&& !strcmp(elem->next->next->next->next->next->next->content, "Hello") \
		&& !strcmp(elem->next->next->next->next->next->next->next->content, "Hello") \
		&& !strcmp(elem->next->next->next->next->next->next->next->next->content, "Hello") \
		&& !strcmp(elem->next->next->next->next->next->next->next->next->next->content, "Hello") \
		&& !strcmp(elem->next->next->next->next->next->next->next->next->next->next->content, "Hello"))
		printf("\033[92mTest %2.i - OK \033[0m\n", 2);
	else
		printf("\033[91mTest %2.i - KO \033[0m\n", 2);
	free(elem->next);
